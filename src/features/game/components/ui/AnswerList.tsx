        ...
        return (
          <li key={answer.id} className="border-line border-t first:border-t-0">
            <button
              ...
              className={cn(
                "group relative flex w-full cursor-pointer items-center gap-3 px-5 py-4 text-left",
                "ease-smooth transition-colors duration-[450ms]",
                "hover:bg-surface-raised focus-visible:bg-surface-raised",
                "disabled:cursor-default disabled:hover:bg-transparent",
                isCorrect && "bg-positive/12 disabled:hover:bg-positive/12",
                isWrong && "bg-negative/12",
              )}
            >
              <span
                aria-hidden
                className={cn(
                  ...
                  "ease-smooth transition-colors duration-[450ms]",
                  isCorrect &&
                    "bg-positive text-background outline-transparent",
                  isWrong && "bg-negative text-background outline-transparent",
                )}
              >
                {index + 1}
              </span>
              ...
              <span
                className={cn(
                  "flex-1 text-base",
                  isCorrect && "text-positive",
                  isWrong && "text-negative",
                )}
              >
                {answer.label}
              </span>
...
function CrossGlyph() {
  return (
    <svg
      ...
      fill="none"
      aria-hidden
      className="text-negative size-4"
    >
      <path
